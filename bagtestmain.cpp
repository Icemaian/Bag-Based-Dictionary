...
int main(int argc, char** argv) {
	cout << "<Isaac Craig> -- CSIS 215 Programming Assignment 1 -- Bag Dictionary" << endl << endl;
	...
	KVpair<int, string> *kvPntr = new KVpair<int, string>(1, "Isaac"); //test KV pointers
	KVpair<int, string> KVtest(2, "dave");

	// test addItem, remove, removeTop, find, += operator, and inspectTop
	cout << "Current Bag size: " << myBag.size() << endl; 
	cout <<"Now adding two more KVpairs" << endl; 
	myBag.addItem(*kvPntr);
	myBag.addItem(KVtest);
	cout << "Current Bag size: " << myBag.size() << endl;
	if (myBag.find(KVtest)) { //tests Find
		cout << "found " << KVtest.value() << endl;
	}
	if (myBag.inspectTop(*kvPntr)) {//tests inspect top
		cout << "InspectTop works" << endl;
	}
	if (myBag.remove(*kvPntr)) {//tests remove
		cout << "remove function works" << endl;
	}
	if (myBag.removeTop(*kvPntr)) {//tests removeTop
		cout << "removeTop function works" << endl;
	}
	myBag+=*kvPntr; //tests overloaded operator
	cout << myBag.size() << " += operator works" << endl;
	myBag.emptyBag(); //tests emptyBag
	cout << myBag.size() << " empty bag works" << endl;
	// end of Bag test
